...
-(NSDictionary *)saveEvent:(id)obj
{
    ...
    NSNumber *recurrenceCount = [self valueForUndefinedKey:@"recurrenceCount"]; 
    EKRecurrenceEnd *end = [EKRecurrenceEnd recurrenceEndWithOccurrenceCount:[recurrenceCount intValue]];
    NSNumber *recurrenceInterval = [self valueForUndefinedKey:@"recurrenceInterval"]; 
    EKRecurrenceRule *rule = [[EKRecurrenceRule alloc]
                              initRecurrenceWithFrequency:EKRecurrenceFrequencyWeekly
                              interval:[recurrenceInterval intValue]
                              end:end];
...
{
    EKEventStore *eventStore = [[[EKEventStore alloc] init] autorelease];
    NSString *eventIdentifier = [self valueForUndefinedKey:@"eventIdentifier"];
	EKEvent *_event = [eventStore eventWithIdentifier: eventIdentifier];
	// not setting this causes all SORTS of explosions when you try to call localizedDescription later
	NSError *err = nil;
	if (_event != nil) {
		[eventStore removeEvent:_event span:EKSpanFutureEvents error:&err];
	}
	BOOL status = (err == nil) ? TRUE : FALSE;
	NSString *errStr = (err != nil) ? [err localizedDescription] : @"none";	
	NSDictionary *tmp = [[[NSDictionary alloc] initWithObjectsAndKeys: errStr, @"error", 
